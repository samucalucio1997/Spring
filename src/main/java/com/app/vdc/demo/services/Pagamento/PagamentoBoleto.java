...
@Service
public class PagamentoBoleto implements PagamentoIS {
	...
	public ClienteAsaas criarCliente(UsuarioSalvo usuario) {
		try {
			...
			ClienteAsaas bodyRequest = new ClienteAsaas(usuario.getUsername(), usuario.getCpfCnpj(),
					usuario.getTelefone(), usuario.getEmail());
			HttpEntity<ClienteAsaas> requestEntity = new HttpEntity<>(bodyRequest, headers);

			ResponseEntity<ClienteAsaas> response = restTemplate.exchange("https://sandbox.asaas.com/api/v3/customers",
					HttpMethod.POST, requestEntity, ClienteAsaas.class);
